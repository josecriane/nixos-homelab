...
# Remove nixos-anywhere temporary keys from SSH agent to avoid auth failures
# (MaxAuthTries=3 + fail2ban would ban us if stale keys are tried first)
...
for i in $(seq 1 100); do
    ...
    echo "Pending services: $JOBS ($i/100)"
    sleep 10
done
