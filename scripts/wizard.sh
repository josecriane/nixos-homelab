...
step_authentik_setup() {
    ...
    if [ -n "$USER_PK" ]; then
        ...
        # Add to all relevant groups (exact match with ?name=)
        for grp in admins media-admins media-users monitoring; do
            GRP_PK=$(curl -sk "$AUTHENTIK_URL/api/v3/core/groups/?name=$grp" \
                -H "$AUTH_HEADER" 2>/dev/null | jq -r '.results[0].pk // empty' 2>/dev/null || echo "")
            if [ -z "$GRP_PK" ] && [ "$grp" = "admins" ]; then
                # Create admins group if sso.nix hasn't run yet
                GRP_PK=$(curl -sk -X POST "$AUTHENTIK_URL/api/v3/core/groups/" \
                    -H "$AUTH_HEADER" \
                    -H "Content-Type: application/json" \
                    -d '{"name":"admins","is_superuser":true}' 2>/dev/null | jq -r '.pk // empty' 2>/dev/null || echo "")
            fi
            if [ -n "$GRP_PK" ]; then
                curl -sk -X POST "$AUTHENTIK_URL/api/v3/core/groups/$GRP_PK/add_user/" \
                    -H "$AUTH_HEADER" \
                    -H "Content-Type: application/json" \
                    -d "{\"pk\": $USER_PK}" >/dev/null 2>&1
            fi
        done
        echo -e "${GREEN}User added to groups: admins, media-admins, media-users, monitoring.${NC}"
    else
